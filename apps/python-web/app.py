"""
CSE644 Cloud Computing - Assignment 02/03
...
Assignment 03 adds a /metrics endpoint (Prometheus text exposition format)
covering request rate/latency by endpoint plus one business-specific gauge
(notes currently stored), on top of prometheus_client's built-in process
metrics (CPU, RSS, start time).
...
# --- Prometheus metrics (Assignment 03, Requirement 5) ---------------------
# Single gunicorn worker (see Dockerfile) so the default in-process registry
# is accurate - no multiprocess collector / shared directory needed. Labels
# are kept to a fixed, low-cardinality set (this app has no path params).
...
NOTES_STORED = Gauge(
    "app_notes_stored",
    "Number of notes currently persisted under DATA_DIR.",
)
...
def _append_note(text):
    ...
    NOTES_STORED.set(len(_read_notes(limit=10_000)))
...
<body>
  <main class="card">
    <div class="bar"></div>
    <div class="inner">
      <span class="badge">Assignment 03 &middot; Python / Flask on :{{ f.listening_port }}</span>
      ...
      <footer>
        ...
        Liveness: <code>/healthz</code> &middot; Readiness: <code>/readyz</code> &middot;
...
@app.get("/metrics")
def metrics():
    """Prometheus scrape target (Assignment 03, Requirement 5)."""
